	...
	fetch(`${apiUrl}?id=${templateId}`)
		...
		.then(data => {
			if (data.status === 'ok') {
				...
				showMessage(`Template loaded for editing: ${data.template.name}`, 'info');
			} else {
				showMessage('Error loading template: ' + data.message, 'error');
			}
		})
		.catch(error => {
			console.error('Error:', error);
			showMessage('An error occurred while loading template', 'error');
		});
}
