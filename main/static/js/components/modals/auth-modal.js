...
function initNotificationsHandlers() {
	console.log('🔧 Initializing notifications handlers...');
	...
	if (bellBtn && notificationsDropdown) {
		...
		bellBtn.addEventListener("click", function (e) {
			...
			// Инициализируем список уведомлений при первом открытии
			initNotificationsList();
		});

		// Закрытие уведомлений при клике вне их
		document.addEventListener("click", function (e) {
			if (!e.target.closest('.notifications-dropdown') && !e.target.closest('#bell')) {
	...
	console.log('✅ Notifications handlers initialized');
}

/**
 * Инициализирует список уведомлений
 */
